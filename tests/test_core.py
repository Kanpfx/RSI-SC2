...
def files(tmp_path):
    root = tmp_path / "files"
    shutil.copytree(ROOT / "bot", root / "bot", ignore=shutil.ignore_patterns("__pycache__"))
    return root
...
def test_editor_boundaries_and_replacement(files):
    editor = Editor(files)
    ...
    assert editor.search("bot/new.py", "world")["matches"][0]["line"] == 1
    with pytest.raises(ValueError):
...
def test_editor_rejects_symlink(files, tmp_path):
    ...
    link = files / "bot/link.py"
    try:
        link.symlink_to(target)
    except OSError:
        pytest.skip("OS does not permit creating symlinks")
    with pytest.raises(ValueError, match="Symlink"):
        Editor(files).apply_patch(patch_text("bot/link.py", None, "bad"))
    assert target.read_text(encoding="utf-8") == "untouched"


def test_commands_reject_arbitrary_execution(files):
...
def test_archive_and_search(tmp_path):
    ...
    for node in [make_node("crashed", 2, crash=1), make_node("expanded", 3, expanded=True),
        ...
        archive.add(node)
    archive.add(make_node("weak", 0))
    assert archive.select_parent(Random(0), 0)["wins"] == 2
    choices = [archive.select_parent(Random(seed), 1)["id"] for seed in range(30)]
    assert "weak" in choices and "crashed" not in choices and "expanded" not in choices
    assert len({archive.select_parent(Random(seed), 0)["id"] for seed in range(30)}) > 1
    assert archive.select_parent(Random(7)) == archive.select_parent(Random(7))
...
def test_evaluation_five_records(tmp_path, monkeypatch):
    ...
    assert (result["wins"], result["losses"], result["ties"], result["crashes"]) == (1, 3, 0, 1)
    assert result["games"] == 5 and intervals == [3, 3, 3, 3]
    assert [item["result"] for item in result["results"]] == ["win", "loss", "loss", "loss", "crash"]
    ...
    assert sorted(p.name for p in (tmp_path / "results").iterdir()) == [f"game_{n:02d}.json" for n in range(1, 6)]
...
class EvolutionLLM:
    def complete(self, messages, tools=None):
        ...
        assert "feedback/metadata.json" in context["feedback_files"]["matches"]
        ...
        return call("apply_patch", {"patch": patch_text("bot/strategy.py", old, new, 1 if first else 2)})
...
def test_one_round_complete_loop(repo):
    ...
    evolution = Evolution(repo, config, EvolutionLLM(), evaluator=FakeEvaluator(),
                          smoke=lambda *args: {"ok": True, "checks": []})
    ...
    for candidate in (first, second):
        ...
        assert changed == ["bot/strategy.py"]
    ...
    assert state["selections"] == [{"round": 1, "parent_id": seed["id"]}]
    rng_state = evolution.rng.getstate()
    evolution.state("completed", 1)
    assert evolution.rng.getstate() == rng_state
    ...
    assert feedback.search("feedback", "evaluation")["matches"]
...
def test_sc2_logged_error_is_not_a_normal_loss(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="on_start failed"):
        play(ROOT, load_config(ROOT / "config.yaml"))
...
def test_parallel_setup_failure_cancels_other_games(tmp_path, monkeypatch):
    evaluator = Evaluator(load_config(ROOT / "config.yaml"))
    barrier = threading.Barrier(5)
    cancelled = []
...
def test_patch_validates_all_sections_before_writing(files):
    editor = Editor(files)
    ...
    assert not (files / "bot/new.py").exists()
    with pytest.raises(ValueError):
    ...
    assert (files / "bot/new.py").read_text().strip() == "value = 1"
    assert (files / "bot/second.py").read_text().strip() == "value = 2"


def test_patch_multiple_hunks_and_no_final_newline(files):
    editor = Editor(files)
    (files / "bot/new.py").write_text("a\nb\nc\nd\ne", encoding="utf-8")
    patch = ("*** Begin Patch\n*** Update File: bot/new.py\n"
             "@@\n a\n-b\n+B\n+extra\n"
             "@@\n d\n-e\n+E\n*** End Patch\n")
    editor.apply_patch(patch)
    assert (files / "bot/new.py").read_text(encoding="utf-8") == "a\nB\nextra\nc\nd\nE"


def test_file_reads_continue_and_search_feedback(files, tmp_path):
    feedback = tmp_path / "feedback"
    feedback.mkdir()
    value = "x" * 13000
    (feedback / "game_01.process.json").write_text(value, encoding="utf-8")
    editor = Editor(files, feedback)
    assert editor.search("feedback")["matches"] == ["feedback/game_01.process.json"]
    ...
    assert editor.search("bot", "SeedBot", glob="*.py")["matches"][0]["path"] == "bot/main.py"
...
def test_retrieval_ranges_and_history(files, tmp_path):
    output = tmp_path / "run"
    parent = output / "nodes/a0"
    save_json(output / "tree.json", {"nodes": [{"id": "a0"}, {"id": "a1"}]})
    save_json(parent / "node.json", {"id": "a0", "wins": 0})
    save_json(output / "nodes/a1/node.json", {"id": "a1", "wins": 1})
    editor = Editor(files, parent)
    name = "feedback/history/a1/node.json"
    assert json.loads(editor.read_file(name)["text"])["wins"] == 1
    with pytest.raises(ValueError):
        editor.apply_patch(patch_text(name, None, "bad"))
    page = editor.search("feedback", glob="*/node.json", limit=1)
    assert page["next_offset"] == 1
    assert editor.search("feedback", glob="*/node.json", offset=1, limit=1)["next_offset"] is None
    whole = editor.read_file("bot/main.py")["text"]
    assert editor.read_file("bot/main.py", offset=2, limit=10)["text"] == whole[2:12]
    with pytest.raises(ValueError):
        editor.read_file("bot/main.py", limit=0)


def test_context_prunes_only_old_retrievals():
    from rsi.context.window import working_messages

    messages = [{"role": "system", "content": "keep rules"}]
    for i in range(8):
        messages.append(call("read_file", {"path": "bot/main.py"}, str(i)))
        messages.append({"role": "tool", "tool_call_id": str(i),
                         "content": json.dumps({"text": "x" * 2000})})
    original = copy.deepcopy(messages)
    working = working_messages(messages, threshold=1)
    assert messages == original
    assert "omitted" in json.loads(working[2]["content"])
    assert working[-12:] == messages[-12:]
    assert [m.get("tool_call_id") for m in working] == [m.get("tool_call_id") for m in messages]
